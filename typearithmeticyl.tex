...
\frame
{
	\begin{proof}[Proof of Distribution: $(\red{a} + \blue{b}) \times c \cong (\red{a} \times c) + (\blue{b} \times c)$]
		\[
		\begin{aligned}
			(\red{a} + \blue{b}) \times c &\cong \cc((\red{a} + \blue{b}) \times c, x) & - yoneda \\
			...
			& \cong \cc(\red{a}  \times c) + (\blue{b} \times c) &- yoneda
		\end{aligned}
		\]
	\end{proof}
}
\frame
{
\begin{definition}[Different Yoneda lemmas]
\begin{tabular}{rc}
Covariant functor:     & $(\forall x. (\red{a} \to x) \to f~x) \cong f~\red{a}$. \\

Contravariant functor: & $(\forall x. (x \to \blue{a}) \to f~x) \cong f~\blue{a}$.
\end{tabular}
\end{definition}
}
...
\frame
{
\begin{definition}[Yoneda (Propositional Equality)]
